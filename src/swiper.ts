...
export class SwiperItem extends MyDisplay {
  ...
  constructor(opt: DisplayConstructor) {
    ...
    const bg = this.qs('.bg');
    const ang = this._id * 10
    // const size = Util.map(Math.sin(Util.radian(ang)), 1, 30, -1, 1)
    const radius = Util.map(Math.sin(Util.radian(ang)), 5, 30, -1, 1)
    const num = 1
    for(let i = 0; i < num; i++) {
      const el = document.createElement('div');
      bg.appendChild(el);
      const rad = Util.radian(ang * -1 + (360 / num * i))
      // const x = 50 + Math.sin(rad) * size - radius * 1;
      const y = 50 - radius + Math.sin(rad) * 50;
      const x = 50 - radius
      // const y = 50 - radius
      Tween.set(el, {
        left: x + '%',
        ...
        backgroundColor: `hsl(${Util.map(this._id, 0, 360 * 2, 0, Conf.ITEM_NUM - 1)}, 100%, 50%)`,
      });
    }
  }
